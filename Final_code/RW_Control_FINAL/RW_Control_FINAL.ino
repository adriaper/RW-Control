...
void set_impulse(bool RW_direction, int New_RW_speed)
{
  ...
  */
  ...
  {
    DriverOne.setDrive(1, 1, New_RW_speed); // Change direction depending on motor connection
    RW_speed = New_RW_speed;
  }
  else // Reaction Wheel Clock Wise
  {
    DriverOne.setDrive(1, 0, New_RW_speed);
    RW_speed = -New_RW_speed;
  }
}
...
void setup()
{
  ...
  IMU.setMagCalX(17.96, 0.79); // The first value corresponds to the MagBias, and the second the ScaleFactor.
  IMU.setMagCalY(6.52, 1.60);
  IMU.setMagCalZ(-38.41, 0.90);
